...
public abstract class ShoulderSurfingCompatMixinPlugin implements IMixinConfigPlugin
{
	...
	private static boolean shouldApplyEpicFightMixins()
	{
		...
		return (major == 21 || minor == 20) && minor < 14;
	}
	...
	private static void addEpicFightMixins(List<String> mixins)
	{
		if(Mods.EPIC_FIGHT.isLoaded())
		{
			if(shouldApplyEpicFightMixins())
			{
				...
				ShoulderSurfingCommon.LOGGER.info("Applying Epic Fight mixins to enable lock-on support for Epic Fight integration.");
			}
			else
			{
				ShoulderSurfingCommon.LOGGER.info("Epic Fight integration has been disabled due to the recent updates and breakage.");
			}
		}
	}
