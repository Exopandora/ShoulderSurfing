...
public class ShoulderRenderer
{
	...
	public void offsetCamera(ActiveRenderInfo camera, World level, float partialTick)
	{
		...
		if(instance.doShoulderSurfing() && level != null && !(camera.getEntity() instanceof LivingEntity && ((LivingEntity) camera.getEntity()).isSleeping()))
		{
	...
	public void updateDynamicRaytrace(ActiveRenderInfo camera, Matrix4f modelViewMatrix, Matrix4f projectionMatrix, float partialTick)
	{
		if(ShoulderInstance.getInstance().doShoulderSurfing())
		{
			...
			RayTraceResult hitResult = ShoulderRayTracer.traceBlocksAndEntities(camera, gameMode, this.getPlayerReach(), RayTraceContext.FluidMode.NONE, partialTick, true, false);
