...
public class ShoulderSurfingWthitPlugin implements IWailaPlugin
{
	...
	private static class ShoulderSurfingObjectPicker implements IObjectPicker
	{
		...
		public void pick(IPickerAccessor accessor, IPickerResults results, IPluginConfig config)
		{
			...
			if(ShoulderInstance.getInstance().doShoulderSurfing() && minecraft.player != null)
			{
				...
				HitResult result = ShoulderRayTracer.traceBlocksAndEntities(camera, minecraft.player, accessor.getMaxDistance(), fluidContext, accessor.getFrameDelta(), traceEntities, !isCrosshairDynamic);
