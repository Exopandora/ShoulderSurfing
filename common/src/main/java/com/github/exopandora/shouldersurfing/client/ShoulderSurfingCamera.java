...
public class ShoulderSurfingCamera implements IShoulderSurfingCamera
{
	...
	public Vector3d calcOffset(ActiveRenderInfo camera, IBlockReader level, float partialTick, Entity cameraEntity)
	{
		...
		if(cameraEntity.isPassenger())
		{
			...
			Vector3d delta = defaultOffset.multiply(passengerOffsetMultipliers).subtract(defaultOffset);
		...
		if(cameraEntity.isSprinting())
		{
			...
			Vector3d delta = defaultOffset.multiply(sprintOffsetMultipliers).subtract(defaultOffset);
		...
		if(this.instance.isAiming())
		{
			...
			Vector3d delta = defaultOffset.multiply(aimingOffsetMultipliers).subtract(defaultOffset);
			targetOffset = targetOffset.add(delta).add(Config.CLIENT.getAimingOffsetModifiers());
		}
		
		if(cameraEntity instanceof LivingEntity && ((LivingEntity) cameraEntity).isFallFlying())
		{
			Vector3d fallFlyingOffsetMultipliers = Config.CLIENT.getFallFlyingMultipliers();
			Vector3d delta = defaultOffset.multiply(fallFlyingOffsetMultipliers).subtract(defaultOffset);
		...
		double targetOffsetX = Config.CLIENT.isUnlimitedOffsetX() ? targetOffset.x() : MathHelper.clamp(targetOffset.x(), Config.CLIENT.getMinOffsetX(), Config.CLIENT.getMaxOffsetX());
		double targetOffsetY = Config.CLIENT.isUnlimitedOffsetY() ? targetOffset.y() : MathHelper.clamp(targetOffset.y(), Config.CLIENT.getMinOffsetY(), Config.CLIENT.getMaxOffsetY());
		double targetOffsetZ = Config.CLIENT.isUnlimitedOffsetZ() ? targetOffset.z() : MathHelper.clamp(targetOffset.z(), Config.CLIENT.getMinOffsetZ(), Config.CLIENT.getMaxOffsetZ());
		targetOffset = new Vector3d(targetOffsetX, targetOffsetY, targetOffsetZ);
	...
	public boolean turn(ClientPlayerEntity player, double yRot, double xRot)
	{
		if(this.instance.isShoulderSurfing())
		{
			...
			if(Config.CLIENT.isCameraDecoupled())
			{
				...
				if(this.instance.shouldEntityFollowCamera(player))
				{
					...
					player.yRotO += MathHelper.degreesDifference(this.yRot, cameraYRot);
				}
				else if(!this.instance.shouldEntityAimAtTarget(player, Minecraft.getInstance()))
				{
					...
					if(Config.CLIENT.shouldPlayerYRotFollowCamera() && !isMoving)
					{
						...
						float playerYRot = MathHelper.approachDegrees(this.lastMovedYRot, player.yRot + scaledYRot, maxFollowAngle);
				...
				if(isMoving)
				{
					this.lastMovedYRot = player.yRot;
				}
			}
