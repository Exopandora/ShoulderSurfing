...
public record Vec2f(float x, float y) {
	...
	public Vec2f rotate(float angle) {
		return new Vec2f(
			this.x * Mth.cos(angle) - this.y * Mth.sin(angle),
			this.x * Mth.sin(angle) + this.y * Mth.cos(angle)
		);
	}
	...
	public Vec2f lerp(Vec2f vec, float f) {
		return new Vec2f(
			Mth.lerp(f, this.x, vec.x),
			Mth.lerp(f, this.y, vec.y)
		);
	}
	
	public Vec2f rotLerp(Vec2f vec, float f) {
		return new Vec2f(
			Mth.rotLerp(f, this.x, vec.x),
			Mth.rotLerp(f, this.y, vec.y)
		);
	}
	
	public Vec2f clampX(float min, float max) {
		return new Vec2f(
			Mth.clamp(this.x, min, max),
			this.y
		);
	}
