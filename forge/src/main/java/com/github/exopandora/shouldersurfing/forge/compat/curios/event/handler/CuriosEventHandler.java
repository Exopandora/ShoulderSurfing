package com.github.exopandora.shouldersurfing.forge.compat.curios.event.handler;
...
public class CuriosEventHandler implements ICuriosEventHandler {
	...
	public void handle(ComputePlayerAimStateEvent event) {
		var lazyOptionalInventory = CuriosApi.getCuriosInventory(event.getEntity());
		if (!lazyOptionalInventory.isPresent()) {
			return;
		}
		var optionalInventory = lazyOptionalInventory.resolve();
		...
		var slotToItemProperties = parseSlots(integrationsConfig.getCuriosAdaptiveCrosshairItemProperties());
		for (var entry : inventory.getCurios().entrySet()) {
			var items = slotToItems.getOrDefault(entry.getKey(), Collections.emptyList());
			var itemProperties = slotToItemProperties.getOrDefault(entry.getKey(), Collections.emptyList());
