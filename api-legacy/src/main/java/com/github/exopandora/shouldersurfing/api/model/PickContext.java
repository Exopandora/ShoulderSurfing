...
public sealed abstract class PickContext permits OffsetPickContext, DynamicPickContext, ObstructionPickContext {
	...
	public com.github.exopandora.shouldersurfing.api.client.world.phys.PickContext toNewApi() {
		if (this instanceof OffsetPickContext offsetPickContext) {
			return new com.github.exopandora.shouldersurfing.api.client.world.phys.OffsetPickContext(
				this.camera,
				this.fluidContext,
				this.entity,
				this.entityFilter,
				offsetPickContext.blockPickOrigin.toNewApi(),
				offsetPickContext.entityPickOrigin.toNewApi()
			);
		} else if (this instanceof DynamicPickContext dynamicPickContext) {
			return new com.github.exopandora.shouldersurfing.api.client.world.phys.DynamicPickContext(
				this.camera,
				this.fluidContext,
				this.entity,
				this.entityFilter,
				dynamicPickContext.pickVector.toNewApi()
			);
		} else if (this instanceof ObstructionPickContext obstructionPickContext) {
			return new com.github.exopandora.shouldersurfing.api.client.world.phys.ObstructionPickContext(
				this.camera,
				this.fluidContext,
				this.entity,
				this.entityFilter,
				obstructionPickContext.endPos
			);
		}
